  ...
  student.findOne({ email }, (err, user) => {
    ...
    if (user.isAlreadyWatching(section)) {
      user.markSectionAsNotNotified(section, (doc) => {
        logger.info(
          `Marked section ${section.department} - ${section.sectionNumber} for ${user.email} as not notified`
        );
    ...
    logger.info(
      `User ${email} is now watching ${section.department} - ${section.sectionNumber}`
    );
...
StudentController.notifyUser = async (user, course, count, paidId, section) => {
  EmailController.sendSpotsAvailableEmail(
    user.email,
  ...
  try {
    if (await PaidIdController.isIdPaid(paidId)) {
      logger.info(
        `Sent text message to ${user.email} for ${course.courseName} - ${course.sectionNumber}`
      );
      ...
      const otherPeople =
        count > 0
          ? `${
              count || "0"
            } other ${personText} ${verbText} watching this section.`
          : "";
      if (!section.phone) {
        logger.info(
          `Section phone invalid, falling back to user.phone: ${user.phone} for ${user.email} - paidId${paidId}`
        );
      }
      TextController.sendMessage(
        section.phone || user.phone,
        `Spots available for section ${course.sectionNumber} in class ${course.courseID}. ${otherPeople}`
      );
    }
  } catch (e) {
    logger.error(
      `Error checking if paid or sending text message for ${user.email} for ${course}- ${e} `
    );
  }
};
...
StudentController.getNumberOfStudentsWatchingSection = async (
  sectionNumber,
  department
) => {
  //Search for users that are being notified for that department
