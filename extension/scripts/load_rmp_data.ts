...
const run = async () => {
  ...
  const existing = await fs.readFile(path.join(__dirname, "../src/data/ratings.json"), "utf-8");
  ...
  await fs.writeFile(path.join(__dirname, "../src/data/ratings.json"), JSON.stringify(professors, null, 4));
};
run();
